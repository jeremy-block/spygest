...
variable_order <- rev(c("Repeated.Phrase", "Content.Verifiability.Errors", "Discourse.Errors", "Semantic.Frame.Errors")) #"No Errors"

# Melt the data frame to long format
data_long <- reshape2::melt(factuality, id.vars = "Participant") %>%
  mutate("Error Type" = variable) %>%
  # Divide all values in the dataframe by 100 to make them a percentatge
